...
function getGroupedQueryParams(req) {
...
function calculateShopifyProxySignature(req, secret) {
  const grouped = getGroupedQueryParams(req);
...
export function verifyShopifyProxyRequest(req) {
  ...
  const signature = getSingleValue(req.query?.signature);
  const hmac = getSingleValue(req.query?.hmac);
...
export function getShopifyCustomerContext(req) {
  ...
  const customerId = getSingleValue(req.query?.logged_in_customer_id);
  const email = getSingleValue(req.query?.customer_email);
...
export function getOptionalShopifyCustomerContext(req) {
  ...
  return {
    ok: true,
    reason: customerId
      ? "Logged-in Shopify customer verified."
      : "Valid Shopify proxy request without logged-in customer.",
